...
/* ── Image input row ── */
.valo-img-row {
  display:flex; align-items:flex-end; gap:8px; margin-bottom:4px; position:relative;
}
...
.valo-cam-btn:hover {
  background:rgba(189,147,249,0.12); border-color:rgba(189,147,249,0.7); box-shadow:0 0 20px rgba(189,147,249,0.3);
}
...
.valo-img-preview img {
  ...
  border:1px solid rgba(189,147,249,0.35); clip-path:polygon(0 0,calc(100% - 5px) 0,100% 5px,100% 100%,0 100%);
}
...
.valo-del-btn {
  background:rgba(255,70,85,0.12);border:1px solid rgba(255,70,85,0.3); color:#ff4655;cursor:pointer;
  width:28px;height:28px; clip-path:polygon(4px 0,100% 0,calc(100% - 4px) 100%,0 100%);
  font-size:14px;display:flex;align-items:center;justify-content:center;
  font-weight:700;transition:all .12s ease;flex-shrink:0;
}
.valo-del-btn:hover {
  background:rgba(255,70,85,0.25);box-shadow:0 0 10px rgba(255,70,85,0.3);
}
/* Send image button */
.valo-send-img-btn {
  background:linear-gradient(90deg,rgba(120,50,210,0.88),rgba(80,30,170,0.82));
  border:1px solid rgba(189,147,249,0.5); border-top:2px solid rgba(200,160,255,0.65);
  clip-path:polygon(0 0,calc(100% - 8px) 0,100% 8px,100% 100%,0 100%);
  color:#fff;cursor:pointer; padding:0 16px;height:46px;
  font-family:'Barlow Condensed',sans-serif; font-weight:800;font-size:12px;letter-spacing:2px;
  white-space:nowrap;flex-shrink:0; box-shadow:0 0 14px rgba(189,147,249,0.2); transition:all .12s ease;
}
.valo-send-img-btn:hover {
  background:rgba(189,147,249,0.25); box-shadow:0 0 22px rgba(189,147,249,0.4);
}
/* Caption input */
.valo-caption-input {
  flex:1;background:#0b0e16; border:1px solid rgba(189,147,249,0.25); border-bottom:2px solid rgba(189,147,249,0.4);
  clip-path:polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,0 100%); color:#ece8e1;
  font-family:'Barlow',sans-serif;font-size:13px; padding:0 12px;height:46px; caret-color:#bd93f9; outline:none;
}
.valo-caption-input::placeholder {
  color:rgba(170,165,155,0.38);font-style:italic;
}
.valo-caption-input:focus {
  border-bottom-color:#bd93f9;box-shadow:0 3px 14px rgba(189,147,249,0.1);
}
/* Paste hint */
.valo-paste-hint {
  font-family:'Barlow Condensed',sans-serif;font-size:10px; letter-spacing:2px;color:#363a46;text-transform:uppercase; text-align:right;margin-top:2px;padding-right:4px;
}
</style>
""", unsafe_allow_html=True)
...
if st.session_state.img_data:
    ...
    with col_send:
        ...
        if st.button("⚡ PHÂN TÍCH ẢNH", use_container_width=True, key="send_img_btn"):
            ...
            with st.chat_message("user"): st.markdown(q_text)
...
[data-testid="stFileUploaderDropzone"] button {
  ...
  display: flex !important; align-items: center !important; justify-content: center !important;
  font-size: 22px !important; cursor: pointer !important; color: #bd93f9 !important;
}
...
[data-testid="stFileUploader"] {
  width: 54px !important; flex-shrink: 0 !important;
}
[data-testid="stFileUploader"] section {
  padding: 0 !important;
}
/* Layout: uploader icon + chat input side by side */
[data-testid="stFileUploader"] + div, [data-testid="stChatInput"] {
  flex: 1 !important;
}
</style>
""", unsafe_allow_html=True)
