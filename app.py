...
# JS hỗ trợ dán ảnh trực tiếp (Ctrl + V)
components.html("""
<script>
...
if st.session_state.img_data:
    ...
    <div class="valo-img-preview">
      ...
      <div class="valo-img-preview-info">
        <strong>📷 Ảnh đã chọn</strong><br>
        {st.session_state.img_name}
      </div>
      <div onclick="document.getElementById('del_img_btn').click()" class="valo-del-btn" style="cursor:pointer;">✕</div>
    </div>
    """, unsafe_allow_html=True)

    col_cap, col_send = st.columns([4, 1])
    with col_cap:
        cap = st.text_input(
            "Ghi chú cho ảnh",
            value=st.session_state.img_caption,
            placeholder="Mô tả lỗi hoặc câu hỏi thêm...",
            label_visibility="collapsed",
            key="caption_input"
        )
        st.session_state.img_caption = cap

    with col_send:
        if st.button("✕ XÓA ẢNH", key="del_img_btn", use_container_width=True):
...
# ── Input row: Nút + bên trái + Chat input ──
c1, c2 = st.columns([1, 12], gap="small")

with c1:
    st.markdown("""
    <div class="valo-cam-btn" title="Thêm ảnh hoặc Ctrl+V dán ảnh">
        <span class="valo-cam-icon">+</span>
    </div>
    """, unsafe_allow_html=True)
    
    # Uploader nằm đè lên nút +
...
# Gửi ảnh + phân tích
if st.session_state.img_data and st.button("⚡ PHÂN TÍCH ẢNH NÀY", type="primary", use_container_width=True):
